...
const GameView: React.FC<GameViewProps> = ({ mode, playerElo, onBack, onUpdateStats }) => {
    ...
    if (option === targetWord) {
      ...
      setTimeout(() => {
        ...
        if (nextIdx < quizQueue.length) {
          ...
          onUpdateStats({ coins: quizScore + 10 }); // End of game bonus
        }
      }, 1000);
    } else {
  ...
  const handleGameOver = (winPlayer: string) => {
    ...
    if (winPlayer === 'You') {
      onUpdateStats({ 
        ...
        coins: 50 + (mode === GameMode.COMPUTER ? 20 : 0) 
      });
    } else {
      onUpdateStats({ 
        losses: 1, 
  ...
  const handleHint = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model: MODEL_NAME,
        contents: `Act as a word chain game assistant. Give me ONLY ONE common English word that starts with the letter "${startChar.toUpperCase()}". 
                  The word must NOT be in this list: [${usedWords}]. 
                  Response should be just the word itself, lowercase, no punctuation.`,
      });
      const hint = response.text.trim().toLowerCase().replace(/[^a-z]/g, '');
      if (hint) {
